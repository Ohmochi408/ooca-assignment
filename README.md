...
3. **Ethical AI Assistance (Not Interpretation):**  
   The browser's speech-to-text (Web Speech API, Thai or English) turns the voice into words while recording. The thought is **named from its first words** and, for voices of 5 seconds or more, keeps up to **three key points — phrases the person actually said** (`src/utils/aiSummary.js`), useful for people who talk a lot. It does **NOT** diagnose, predict emotional states, or name an emotion. The name can always be changed; if speech-to-text misheard, *“Not right? Remove”* takes the points away (with Undo). The words themselves are not stored. When the browser can't listen (e.g. Firefox), the screen says so instead of inventing a summary. A production version could pass the same words to a language model for a rewritten summary under the same rules.
4. **The Sky Concept:**  
