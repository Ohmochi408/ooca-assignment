...
function blobToPath(bytes, m) {
  ...
  for (let o = 0; o < buf.length; ) {
    const cmd = buf[o++];
    if (cmd === 0) d += 'Z';
    else if (cmd === 1) { d += `M${pt(o)}`; o += 8; }
    else if (cmd === 2) { d += `L${pt(o)}`; o += 8; }
    else if (cmd === 3) { d += `Q${pt(o)} ${pt(o + 8)}`; o += 16; }
    else if (cmd === 4) { d += `C${pt(o)} ${pt(o + 8)} ${pt(o + 16)}`; o += 24; }
...
function collect(node, matrix, out, isRoot = false) {
  ...
  if (!isRoot || node.type !== 'SYMBOL') {
    ...
    if (visible(node.strokePaints) && node.strokeWeight > 0) for (const g of node.strokeGeometry ?? []) out.push({ d: blobToPath(blobs[g.commandsBlob].bytes, m), rule: g.windingRule });
  }
...
for (const [slug, name] of Object.entries(ICONS)) {
  ...
  if (!sym) { missing.push(name); continue; }
  const paths = [];
