    ...
    <Sheet title="Choose a day and sky" onClose={onClose}>
      <div className="flex items-center justify-between mb-3">
        <button
          ...
          className="w-11 h-11 rounded-full flex items-center justify-center text-bluegray-600 hover:bg-gray-100 cursor-pointer"
        ...
        <button
          ...
          className="w-11 h-11 rounded-full flex items-center justify-center text-bluegray-600 hover:bg-gray-100 disabled:opacity-30 cursor-pointer"
      ...
      <div className="grid grid-cols-7 gap-1 text-center mb-5">
        {weekdays.map((w, i) => (
          <span key={i} className="text-body5 text-bluegray-600 py-1">
            {w}
          </span>
        ))}
        {cells.map((k, i) =>
          k ? (
            ...
            >
              {Number(k.slice(-2))}
              {byDay[k] && (
                <span className={`absolute bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full ${day === k ? 'bg-white' : 'bg-turquoise-500'}`} />
              )}
            </button>
          ) : (
            <span key={`e${i}`} />
      ...
      <div className="flex items-center gap-3">
        <button onClick={() => onApply(today, getSkyPeriod())} className="ooca-btn ooca-btn-secondary ooca-btn-turquoise flex-1">
          Now
        </button>
        <button onClick={() => onApply(day, sky)} className="ooca-btn ooca-btn-primary ooca-btn-turquoise flex-[2]">
          Show this sky
        </button>
      </div>
    </Sheet>
